...
export const generateVideo = async (
  ...
  let operation = await ai.models.generateVideos({
    ...
    prompt: prompt,
    image: image ? { imageBytes: image.data, mimeType: image.type } : undefined,
    config: {
      numberOfVideos: 1,
    }
  });
  ...
  onStatusUpdate("Fetching video data...");
  const response = await fetch(`${downloadLink}&key=${process.env.API_KEY}`);
  if (!response.ok) {
      throw new Error(`Failed to fetch video from download link. Status: ${response.statusText}`);
  }
